...
class ArenaCategory:
	...
	# 1f. In case player files exist and may be outdated, update them from past tournaments.
	def _FixPlayersUpdate(self):
		
		# First load list of relevant players to maintain
		self.PrintMessage(f"Loading list of relevant players...")
		with open(f"{self._PathPlayers}{self._V}_{self._E}.txt", "r") as UsernameFile:
			for Line in UsernameFile:
				self._PlayerList[Line.strip().lower()] = 1
		...
		AlreadyProcessedIDs = dict()
		with open(f"{self._PathPlayers}{self._V}_{self._E}__events.txt", "r") as ArenaIDFile:
			for Line in ArenaIDFile:
				AlreadyProcessedIDs[Line.strip().lower()] = 1
		self.PrintMessage(f"Loaded {len(AlreadyProcessedIDs)} preprocessed arena IDs into a list.")		

		# If list already up to date, then skip
		if len(AlreadyProcessedIDs) == len(self._RankingList):
			self.PrintMessage(f"Already up to date! Skipping.")
			return
		
		# For all events not yet processed, go through them to update
		
		
		# Store new data to files
